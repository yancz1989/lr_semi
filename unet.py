# -*- coding: utf-8 -*-
# @Author: yancz1989
# @Date:   2017-05-08 11:23:41
# @Last Modified by:   yancz1989
# @Last Modified time: 2017-06-08 11:17:17
import settings
import numpy as np
import mxnet as mx
import logging
...
def convolution_module(net, kernel_size, pad_size, filter_count, stride=(1, 1), work_space=2048, batch_norm=True, down_pool=False, up_pool=False, act_type="relu", convolution=True):
  if up_pool:
    net = mx.sym.Deconvolution(net, kernel=(2, 2), pad=(0, 0), stride=(
        2, 2), num_filter=filter_count, workspace=work_space)
  ...
  if convolution:
    conv = mx.sym.Convolution(data=net, kernel=kernel_size, stride=stride,
...
def get_net_180():
  ...
  pad_size = (1, 1)
  filter_count = 32
  pool1 = convolution_module(source, kernel_size, pad_size,
                             filter_count=filter_count, down_pool=True)
  net = pool1
  pool2 = convolution_module(net, kernel_size, pad_size,
                             filter_count=filter_count * 2, down_pool=True)
  net = pool2
  pool3 = convolution_module(net, kernel_size, pad_size,
                             filter_count=filter_count * 4, down_pool=True)
  net = pool3
  ...
  pool5 = convolution_module(net, kernel_size, pad_size,
                             filter_count=filter_count * 8, down_pool=True)
  net = pool5
  ...
  # dirty "CROP" to wanted size... I was on old MxNet branch so user conv
  # instead of crop for cropping
  ...
  net = convolution_module(net, kernel_size, pad_size,
                           filter_count=filter_count * 2, up_pool=True)

  net = convolution_module(net, kernel_size, pad_size,
                           filter_count=1, batch_norm=False, act_type="")
  print_inferred_shape(net)
...
if __name__ == "__main__":
  ...
  for fold_no in range(0, settings.FOLD_COUNT):
    ...
    data_train = FileIter(root_dir=settings.BASE_TRAIN_SEGMENT_DIR, flist_name="train" + str(fold_no) + ".lst", batch_size=BATCH_SIZE, augment=AUGMENT_TRAIN,
                          mean_image=settings.BASE_TRAIN_SEGMENT_DIR + "mean.png", random_crop=True, crop_size=INPUT_SIZE, shuffle=True, scale_size=None)
    data_validate = FileIter(root_dir=settings.BASE_TRAIN_SEGMENT_DIR, flist_name="validate" + str(fold_no) + ".lst", batch_size=BATCH_SIZE,
                             augment=False, mean_image=settings.BASE_TRAIN_SEGMENT_DIR + "mean.png", crop_size=INPUT_SIZE, scale_size=None)
    train_model.fit(
        X=data_train,
        eval_data=data_validate,
        eval_metric=eval_metric,
        epoch_end_callback=mx.callback.do_checkpoint(
            MODEL_PREFIX + "fold" + str(fold_no))
    )
